...
class HockeyHighlightExtractor:
    """
    End-to-end extractor with all methods defined on the class.
    Drop this above `main()` and remove any "attach functions to class" loop.
    """
    def __init__(self, video_path, folders):
        from pathlib import Path
        self.video_path = Path(video_path)
        self.folders = folders  # expects keys like 'output_dir' (and/or 'game_dir')
    ...
    def create_highlights(self,
        ...
        # Merge nearby hits so we don’t duplicate
        merged = []
        last_t = None
        for ev in events:
        ...
        if subclips:
            ...
            try:
                ...
                # Close subclips/concat to release handles (Windows)
                try:
                    concat.close()
                except Exception:
                    pass
    ...
    def generate_report(self, game_meta=None):
        """
        Write a CSV/JSON summary of detected events to folders['output_dir'] (if provided),
        else into folders['game_dir'].
        """
        import csv, json
        from pathlib import Path
