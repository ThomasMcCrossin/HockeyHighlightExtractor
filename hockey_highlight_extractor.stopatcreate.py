...
from config import *
# [ADD BELOW THIS LINE 43]
# Temp files: always local to avoid Google Drive sync/permission issues
TEMP_DIR = LOCAL_REPO_DIR / "temp"
TEMP_DIR.mkdir(parents=True, exist_ok=True)

import os
os.environ.setdefault("TMPDIR", str(TEMP_DIR))  # POSIX temp
os.environ.setdefault("TEMP", str(TEMP_DIR))    # Windows temp
os.environ.setdefault("TMP", str(TEMP_DIR))     # Windows/legacy
import tempfile
tempfile.tempdir = str(TEMP_DIR)
# [ADD ABOVE THIS LINE 45]
...
def load_video_and_audio(self):
    ...
    def create_highlights(self):
        ...
        if not all_events:
            ...
            try:
                test_clip = subclip_compat(self.video_clip, 60, min(80, self.video_clip.duration))
        ...
        for i, event in enumerate(selected_events):
            ...
            try:
                ...
                clip = safe_operation(f"subclip_{i+1}", subclip_compat, self.video_clip, start_time, end_time)
                clips.append(clip)
...
class HockeyHighlightExtractor:
    """
    Goal-biased hockey highlight extractor.
    - Writes individuals to <output_dir>/clips/
    - Always writes stitched reel to <output_dir>/highlights.mp4
    - Prefers goals via (announcer ∧ crowd) + optional horn cue
    - Filters out low-motion (intermission/static) windows
    Requires helpers already in this file: logger, safe_operation, write_videofile_compat, write_audiofile_compat, clean_audio_data
    """
    def __init__(self, video_path, folders):
        from pathlib import Path
        self.video_path = Path(video_path)
        self.folders = folders  # expects 'output_dir' and/or 'game_dir'
        self.video_clip = None
        self.audio_data = None            # mono float32 [-1,1]
        self.audio_sr = globals().get("AUDIO_SR", 22050)
        self.speech_segments = []         # list[(start,end)]
    ...
    def load_video_and_audio(self):
        ...
        # Extract audio to temp and read robustly (soundfile -> scipy fallback)
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                if original_sr != self.audio_sr:
                    try:
                        import librosa
                        y = librosa.resample(y, orig_sr=int(original_sr), target_sr=int(self.audio_sr), res_type="kaiser_fast")
                    except Exception as e2:
                        logger.warning(f"librosa resample failed: {e2}; using scipy.signal")
                        from scipy.signal import resample_poly
                        from math import gcd
                        g = gcd(int(original_sr), int(self.audio_sr))
                        up, down = int(self.audio_sr // g), int(original_sr // g)
    ...
    def detect_speech_segments(self,
        ...
        if globals().get("VAD_AVAILABLE", False):
            # WebRTC VAD path (convert to 16 kHz, 16-bit PCM)
            try:
        ...
        logger.info(f"🗣️ Detected {len(segments)} speech segments (≈{sum(e-s for s,e in segments):.1f}s).")
        return segments
    ...
    def analyze_announcer_excitement(self,
        ...
        def rz(x):
            ...
            return (x - med) / mad
        score = 0.7 * rz(hi_energy) + 0.3 * rz(centroid)
        ...
        events = []
        last_t = -1e9
        thr = np.nanpercentile(score_masked[np.isfinite(score_masked)], 85)
    ...
    def analyze_crowd_reactions(self,
                                ...
                                top_k=10):
        """
        Detect broadband 'cheer' moments via RMS + spectral flatness.
        ...
        thr = np.nanpercentile(cheer_score[np.isfinite(cheer_score)], 90)
        events, last_t = [], -1e9
    ...
    # ------------------------------
    # UTIL: LOW-MOTION FILTER (intermissions/static)
    # ------------------------------
    def _low_motion_windows(self, sample_fps=2, win_s=6.0, thr=0.12):
        """
        Return [(start,end), ...] intervals with very low motion (likely intermission/static).
        """
        ...
        for t in times:
            try:
                frame = V.get_frame(float(t))
                gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
                small = cv2.resize(gray, (160, 90), interpolation=cv2.INTER_AREA)
                if prev is not None:
                    diffs.append(np.mean(np.abs(small.astype(np.float32) - prev)))
                prev = small
            except Exception:
                diffs.append(0.0)
        if not diffs:
        ...
        m = np.median(diffs); mad = np.median(np.abs(diffs - m)) + 1e-6
        norm = (diffs - m) / mad
        low = norm < thr
        windows, start_idx = [], None
        min_len = max(2, int(win_s / step))
        for i, is_low in enumerate(low):
            if is_low and start_idx is None:
                ...
                if i - start_idx >= min_len:
                    s = float(times[start_idx]); e = float(times[min(i, len(times)-1)])
                    windows.append((s, e))
                start_idx = None
        if start_idx is not None and len(low) - start_idx >= min_len:
            windows.append((float(times[start_idx]), float(times[-1])))
        return windows

    # ------------------------------
    # UTIL: GOAL HORN CUE
    # ------------------------------
    def _goal_horn_peaks(self, band=(350, 800), frame_ms=100, hop_ms=50, min_sep_s=6.0):
        """
        Look for sustained energy in typical horn band. Returns [{'t': float, 'score': float}, ...].
        """
        import numpy as np, librosa
        y, sr = self.audio_data, self.audio_sr
        if y is None:
            return []
        n_fft = int(sr * frame_ms / 1000.0)
        hop = int(sr * hop_ms / 1000.0)
        ...
        band_en = S[mask].mean(axis=0)
        med = np.median(band_en); mad = np.median(np.abs(band_en - med)) + 1e-8
        z = (band_en - med) / mad
        t_axis = np.arange(S.shape[1]) * (hop / sr)
        events, last_t = [], -1e9
        thr = np.nanpercentile(z[np.isfinite(z)], 92)
        for t, sc in zip(t_axis, z):
            if sc >= thr and (t - last_t) >= min_sep_s:
                last_t = float(t)
                events.append({"t": last_t, "score": float(sc)})
        return events

    # ------------------------------
    # UTIL: GOAL-FOCUSED CANDIDATES
    # ------------------------------
    def pick_goal_candidates(self, window_join_s=2.5, use_horn=True):
        """
        Build candidates where announcer excitement AND crowd cheer overlap within window_join_s.
        Optionally add horn hits with a score boost.
        """
        A = self.announcer_events or []
        C = self.crowd_events or []
        if not A and not C:
            return []
        merged = []
        i = j = 0
        while i < len(A) and j < len(C):
            ta, tc = A[i]['t'], C[j]['t']
            if abs(ta - tc) <= window_join_s:
                sc = 0.6*A[i]['score'] + 0.6*C[j]['score']
                merged.append({"t": float((ta+tc)/2.0), "score": float(sc)})
                i += 1; j += 1
            elif ta < tc:
                i += 1
            else:
                j += 1
        if use_horn:
            for ev in self._goal_horn_peaks():
                merged.append({"t": ev["t"], "score": ev["score"] + 2.0})
        # time-NMS by best score
    ...
    def create_highlights(self,
        ...
        """
        Save per-clip MP4s under <output_dir>/clips and ALWAYS write a stitched reel to
        <output_dir>/highlights.mp4. Uses goal-biased candidate selection + low-motion filter.
        """
        from pathlib import Path
        ...
        output_dir = Path(self.folders.get("output_dir", self.folders.get("game_dir", ".")))
        clips_dir = output_dir / "clips"
        clips_dir.mkdir(parents=True, exist_ok=True)

        # Goal-focused events
        candidates = self.pick_goal_candidates()
        if not candidates:  # fallback if nothing overlaps
            candidates = (self.announcer_events or []) + (self.crowd_events or [])

        # Filter out likely intermission/static windows
        low_motion = self._low_motion_windows()
        def _blocked(t):
            for s, e in low_motion:
                if (s - 1.0) <= t <= (e + 1.0):
                    return True
            return False
        events = [ev for ev in candidates if not _blocked(ev["t"])]
        if not events:
            events = candidates  # absolute fallback

        if not events:
            logger.warning("No events to clip.")
            return []

        # Merge near-duplicates by time gap
        events = sorted(events, key=lambda d: d["t"])
        merged_ts, last_t = [], None
        for ev in events:
            if last_t is None or (ev["t"] - last_t) >= min_gap_s:
                merged_ts.append(ev["t"])
                last_t = ev["t"]
        merged_ts = merged_ts[:max_clips]

        V = self.video_clip
        total = float(V.duration)
        subclips, written_paths = [], []

        for idx, t in enumerate(merged_ts, start=1):
        ...
        # ALWAYS write stitched reel
        if subclips:
            reel_path = output_dir / "highlights.mp4"
            try:
                concat = concatenate_videoclips(subclips, method="compose")
                try:
                    concat.write_videofile(str(reel_path), fps=V.fps, codec="libx264", audio_codec="aac")
                except Exception:
                    write_videofile_compat(concat, str(reel_path))
                written_paths.append(str(reel_path))
            except Exception as e:
                logger.warning(f"Failed to write combined reel: {e}")
            finally:
                try:
                    concat.close()
                except Exception:
        ...
        logger.info(f"🎯 Wrote {len(written_paths)} file(s).")
    ...
    def generate_report(self, game_meta=None):
        """
        Write a CSV/JSON summary to folders['output_dir'] (if provided), else folders['game_dir'].
        """
        import csv, json
        from pathlib import Path

        report_dir = Path(self.folders.get("output_dir", self.folders.get("game_dir", ".")))
...
def main():
    ...
    temp_logger = setup_logging(temp_log_path)
    global logger
    logger = temp_logger
    
    try:
        # Find video in multiple locations
        video_file = find_video_in_project()
        if not video_file:
        ...
        is_from_google_drive = GOOGLE_INPUT_DIR and str(video_path).startswith(str(GOOGLE_INPUT_DIR.parent))
        
        print(f"✅ Found video: {video_path.name}")
        print(f"📍 Source: {'Google Drive' if is_from_google_drive else 'Local'}")
        
        # Parse filename to determine game info
        print("\\n🔍 Parsing game information...")
        
        # Try MHL format first
        game_info = parse_mhl_filename(video_path.name)
        if not game_info:
            game_info = parse_generic_hockey_filename(video_path.name)
        
        print(f"📅 Date: {game_info['date']}")
        print(f"🏒 League: {game_info['league']}")
        print(f"🏠 {game_info['home_team']} vs {game_info['away_team']}")
        print(f"🎯 Perspective: {game_info['home_away'].title()}")
        ...
        log_file = game_folders['logs_dir'] / 'processing_log.txt'
        logger = setup_logging(log_file)
        
        output_type = "Google Drive" if GOOGLE_GAMES_DIR else "Local"
        print(f"\\n📁 Game Folder: {game_folders['folder_name']} ({output_type})")
        print(f"🔍 Debug Logs: {log_file}")
        
        # Initialize extractor
        extractor = HockeyHighlightExtractor(video_file, game_folders)
        
        # Processing steps
        print("\\n" + "=" * 70)
        print("STEP 1: LOADING VIDEO & AUDIO")
        print("=" * 70)
        
        if not extractor.load_video_and_audio():
            print("❌ Video/audio loading failed")
            input("Press Enter to exit...")
            return
        
        print("\\n" + "=" * 70)
        print("STEP 2: SPEECH DETECTION")
        print("=" * 70)
        
        extractor.detect_speech_segments()
        
        print("\\n" + "=" * 70)
        print("STEP 3: ANNOUNCER ANALYSIS")
        print("=" * 70)
        ...
        success = extractor.create_highlights(write_individual=True)
        ...
        # Move original video to source folder (after successful processing)
        if success:
            try:
                source_path = game_folders['source_dir'] / video_path.name
                if not source_path.exists() and video_path != source_path:
                    shutil.move(str(video_path), str(source_path))
                    print(f"📁 Moved video to: {source_path}")
                    logger.info(f"Video moved to source folder: {source_path}")
                else:
                    print(f"📁 Video location: {video_path}")
            except Exception as e:
                logger.warning(f"Could not move video to source folder: {e}")
        
        # Final results
        print("\\n" + "=" * 70)
        print("✅ COMPLETE HOCKEY ANALYSIS FINISHED!")
        print("=" * 70)
        ...
        if success:
            print(f"   🎥 Highlights: {game_folders['output_dir'] / 'highlights.mp4'}")
        print(f"   📊 Analysis: {game_folders['output_dir'] / 'analysis_report.txt'}")
        print(f"   📄 Data Files: {game_folders['output_dir']}")
        print(f"   🔍 Debug Log: {log_file}")
        
        # Cleanup
        extractor.cleanup()
        
        print("\\n🏒 COMPLETE HYBRID BENEFITS:")
        ...
        if success:
            print("\\n🎉 SUCCESS! Complete hockey highlight extraction finished!")
        else:
            print("\\n⚠️ Video creation had issues - check the log file")
